...
export const listenSokCoins = async (binance) => {
    ...
    binance.websockets.chart("BTCUSDT", "1m", (symbol, interval, chart) => {
        ...
        let body = chartData;
        let open = JSON.stringify(body.o[0])
       ...
       let low = JSON.stringify(body.l[0])
        ...
        let volume = JSON.stringify(body.v[0])
        volume = volume.substring(1, (volume.length - 3))
        volume = volume.split(',').map(Number)

        let timeStamp = JSON.stringify(body.t[0])
        ...
        let ME = EMA.calculate({ period: 99, values: candleData.c });
        let BANDS = BB.calculate({ period: 20, stdDev: 2, values: candleData.c });
        //console.info({ ME, BANDS, candleData, })
            ...
            if (err) {
                ...
                binance.exchangeInfo((err, data) => {
                    ...
                    try {
                        ...
                        let amountCalcBuy = priceOrder * amountBuy;
                        let amountCalcSell = priceOrder * amountSell;
